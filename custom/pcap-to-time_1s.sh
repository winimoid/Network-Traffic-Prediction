...
if [ "$#" -ne 2 ]; then
  echo "${magenta}Utilisation du script : $0 chemin_vers_fichier_pcap chemin_de_sauvegarde${reset_color}"
  exit 1
fi
...
for pcap_file in "${pcap_files[@]}"; do
    ...
    # Utilisation de tshark pour extraire la longueur des paquets par seconde et sauvegarder dans un fichier sans extension
    tshark -r "$pcap_file" -q -z "io,stat,1,frame.len" | grep -v "^===" | awk '{print $8}' > "$output_file"

    echo "${cyan}Traitement terminé pour $pcap_file. Résultats sauvegardés dans $output_file ${reset_color}"
done

echo "${blue}Tous les fichiers PCAP ont été traités.${reset_color}"
