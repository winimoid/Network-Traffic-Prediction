...
# %%
path_result = "results/lstm/timestep_%d/lb=%d_la=%d_ne1=%d_ne2=%d_ne=%d/" % (
timestep, look_back, look_ahead, neuron1, neuron2, neuron3)

if os.path.isdir("results/lstm/timestep_%d" % timestep) == 0:
    os.system("mkdir results/lstm/timestep_%d" % timestep)
...
# plot_series(times, series, ylabel="Packets / minute", path=path_result+"splitted_data.png")
...
model = Sequential()
model.add(LSTM(neuron1, input_dim=1))
...
columns = ["look_back", "look_ahead", "neuron1", "neuron2", "neuron3"] + ["MAPE: %d Step" % i for i in
                                                                          range(1, look_ahead + 1)]
